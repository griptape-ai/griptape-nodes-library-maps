...
class GoogleStreetView(ControlNode):
    ...
    def _handle_api_response(self, response: requests.Response) -> str:
        ...
        image_url = GriptapeNodes.StaticFilesManager().save_static_file(image_data, filename)
        ...
        def fetch_street_view() -> ImageUrlArtifact:
            try:
                ...
                self.publish_update_to_parameter(
                    "status", f"Street View image fetched successfully for: {self.get_parameter_value('address')}"
                )
